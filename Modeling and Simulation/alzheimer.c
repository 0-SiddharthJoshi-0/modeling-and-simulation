...
int main() {
  ...
  double ka = 0.02; // Rate of amyloid-β accumulation
  double kt = 0.03; // Rate of tau phosphorylation
  double d_n = 0.001; // Rate of neuronal death
  double dt = 0.01; // Time step (in years, adjust for desired simulation duration)

  // Simulation duration (adjust for desired time frame)
  double simulation_time = 20.0; // Simulation duration (years)

  // Open file for data storage (modify filename if desired)
  FILE *data_file = fopen("simulation_data8.dat", "w");
